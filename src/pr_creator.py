...
def create_pr(
    ...
    weekly_buffer_path: pathlib.Path,
    week_label: str,
    article_count: int = 0,
    repo_root: str | pathlib.Path | None = None,
    briefing: str = "",
) -> str | None:
    """Create a branch, commit weekly digest files, and open a PR.
    ...
    try:
        ...
        for path in digest_paths:
            _run(["git", "add", str(path)], cwd=cwd)
        _run(["git", "add", str(seen_db_path)], cwd=cwd)
        _run(["git", "add", str(weekly_buffer_path)], cwd=cwd)

        # Commit
        commit_msg = f"Add weekly digest: {week_label}"
        ...
        # Build PR body
        pr_body = _build_pr_body(week_label, article_count, digest_paths, briefing)
...
def _build_pr_body(
    ...
    lines = [
        ...
        f"- **Daily digests**: {len(digest_paths)} files",
    ]
    ...
    lines.extend(["", "---", "", "### Digest files"])
    for p in digest_paths:
        lines.append(f"- `digests/{p.name}`")
